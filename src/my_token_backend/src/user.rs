...
pub fn get_profile_details() -> (String,String,String,String){    
    ...
    let user = match state.all_user.get(&fixed_caller()).cloned(){
        None => UserData{
            name: "".to_string(),
            email: "".to_string()
        },
    ...
    (fixed_caller().to_text(), user.name , user.email , balance.to_string())
}
...
pub fn mint_tokens(amount : String) -> String {
    match mint(convert_from_string_to_u128(amount), fixed_caller()){
        Ok(_a)=> "Success".to_string(),
        Err(_)=> "Failed".to_string()
    }
}
...
pub fn get_header_details() -> (String,String) {
    (fixed_caller().to_text() , check_balance_of(fixed_caller().to_text()))
}
