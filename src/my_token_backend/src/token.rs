...
use crate::fixed_caller::fixed_caller;
...
pub fn transfer(receiver : Principal, amount : u128) -> Result<u128,Errors>{
    ...
    if receiver == fixed_caller() {
        return Err(Errors::ReceiverSameAsSender);
    }

    let mut state = get_state();
    let balance = check_balance(fixed_caller());
    ...
    state.all_users_balance.insert(fixed_caller(), balance - amount);   
...
pub fn update_user_details(username: String, email: String){
    let mut state = get_state();
    state.all_user.insert(fixed_caller(), UserData{ name : username , email : email});
    save_state(state);
}
...
pub fn new_user(){
    let mut state = get_state();
    if state.all_user.contains_key(&fixed_caller()) {
        return;
    };
    if state.all_users_balance.contains_key(&fixed_caller()) {
        return;
    };

    state.all_user.insert(fixed_caller(), UserData{ name : "".to_string() , email : "".to_string()});
    state.all_users_balance.insert(fixed_caller(), 0);
    save_state(state);
}
...
pub fn add_faucets(){
    ...
    let curr_balance = check_balance(fixed_caller());
    state.all_users_balance.insert(fixed_caller(),curr_balance + 100);
